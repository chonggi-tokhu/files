/*
 * ColourgreyShorterCSSJS 0.0.2 (strict data type checking version)
 *
 * developer: Chonggi-tokhu ('colourgrey' on sites other than github)
 * 
 * I replaced all comparison operators that ignore data type (==) with comparison operators that check whether data type equals (===).
 * 
...
(function (gTh, CGRshorter) { "object" === typeof exports && "undefined" != typeof module ? module.exports = CGRshorter() : "function" === typeof define && define.amd ? define(CGRshorter) : (gTh = "undefined" != typeof globalThis ? globalThis : gTh || self).ColourgreyShorterJS = CGRshorter() })(this, function () {
    ...
    function checkObjandItsProperties(param0, [...propnames_and_conditions] = [{ propname: false, condition: {/* checkingType:true,dataTypetoMatch:'object' */ } }]) {
        ...
        if (checkObj(param0)) {
            for (var inc01 = 0, val = propnames_and_conditions_0[inc01], idx = inc01, arr = propnames_and_conditions_0; inc01 < propnames_and_conditions_0.length; inc01++, val = propnames_and_conditions_0[inc01], idx = inc01, arr = propnames_and_conditions_0) {
                if (!checkObjandItsProperty(param0, val.propname, val.condition)) {
                    rtbool = false;
                    break;
                }
            }
    ...
    Document.prototype.setCookie = function (...cookies) {
        for (var i = 0; i < cookies.length; i++) {
            cookies[i] = (checkObjandItsProperty(cookies[i], 'cookie_name', function (checkingstrparam) { return (typeof checkingstrparam === 'string' || checkingstrparam instanceof String) }) && checkObjandItsProperty(cookies[i], 'cookie_value', function (checkingstrparam) { return (typeof checkingstrparam === 'string') })) ? cookies[i] : null;
        }
